...
class _VGGDistance(nn.Module):
    ...
    def forward(self, I1, I2):
        b_sz = I1.size(0) #batch size
        f1 = self.vgg(I1, self.levels)
        f2 = self.vgg(I2, self.levels)
        loss = torch.abs(I1 - I2).view(b_sz, -1).mean(1)
        #loss percptual is the difference on the semantic and patterns between the images.
        for i in range(self.levels):
            # print("shapes:", f1[i].shape) # ([128, 64, 32, 32]), ([128, 128, 16, 16]), ([128, 256, 8, 8])
            layer_loss = torch.abs(f1[i] - f2[i]).view(b_sz, -1).mean(1)
            # print("shapes1:", layer_loss.shape) #shapes1: torch.Size([128])
