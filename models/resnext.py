...
class ResNeXt(nn.Module):
    ...
    def forward(self, x): #TODO change
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        x = self.layer2(x)
        third = x
        return [x, first, second, third, third]
        # x = self.layer3(x)
        # # print("x shape4", x.shape) #x shape4 torch.Size([5, 1024, 4, 4, 4])
        # x = self.layer4(x)
        # # print("x shape5", x.shape) #x shape5 torch.Size([5, 2048, 2, 2, 2])
        # x = self.avgpool(x)
        # x = x.view(x.size(0), -1)
        # # print("x shape6", x.shape) #x shape6 torch.Size([5, 2048])
        # x = self.fc(x)
        # # print("x out", x.shape) #x out torch.Size([5, 400])
