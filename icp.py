...
OptParams = collections.namedtuple('OptParams', 'lr batch_size epochs ' + 'decay_epochs decay_rate ')
OptParams.__new__.__defaults__ = (None, None, None, None, None)


class _netT(nn.Module):
    # xn is the dimension of noise vector (e)
    # yn is the dimension of latent vector (z)
...
class _ICP():
    ...
    def train_epoch(self, z_np, epoch):
        ...
        n, d = z_np.shape # this is the shape of the wieght matrix W (of net Z)
        batch_n = n // batch_size
        rp = np.random.permutation(n) # n is the number of z_i

        # Compute learning rate
        decay_steps = epoch // self.opt_params.decay_epochs
        lr = self.opt_params.lr * self.opt_params.decay_rate ** decay_steps
        # Initialize optimizers
        optimizerT = optim.Adam(self.netT.parameters(), lr=lr,betas=(0.5, 0.999), weight_decay=1e-5)
        ...
        # print("daniel01", Te_np)
        for i in range(M): # this is the stage that we attach to each z_i the closet vector noise
        ...
        for i in range(batch_n):
            ...
            z_est = self.netT(e) # operate T on the noise vector e
...
class ICPTrainer():
    def __init__(self, f_np, d): #f_np is W (of z) , d is the dim of the noise vector(e)
        self.f_np = f_np
